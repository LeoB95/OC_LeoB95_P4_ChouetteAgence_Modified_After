...
const UIStrings = {
  ...
  title:
    "Avoids front-end JavaScript libraries" +
  ...
  failureTitle:
    "Includes front-end JavaScript libraries" +
    " with known security vulnerabilities",
...
class NoVulnerableLibrariesAudit extends Audit {
  ...
  static getVulnerabilities(normalizedVersion, lib, snykDB) {
    ...
    const matchingVulns = vulnCandidatesForLib.filter((vulnCandidate) => {
      ...
      const hasMatchingVersion = vulnCandidate.semver.vulnerable.some(
        (vulnSemverRange) =>
  ...
  static audit(artifacts) {
    const foundLibraries = artifacts.Stacks.filter(
    ...
    const headings = [
      {
        key: "detectedLib",
        itemType: "link",
        text: str_(UIStrings.columnVersion),
      },
      { key: "vulnCount", itemType: "text", text: str_(UIStrings.columnVuln) },
      {
        key: "highestSeverity",
        itemType: "text",
        text: str_(UIStrings.columnSeverity),
      },
    ];
